...
import {
	...
	initialConfirmActionModalState,
...
import { groupBy, pullRequestMetadataText, repositoryOwner } from "./ui/pullRequests.js"
...
export const App = () => {
	...
	const submitIssueCommentViaCommentModal = () => {
		if (!selectedPullRequest) return
	...
	const submitReplyComment = () => {
		...
		const key = pullRequestDiffKey(selectedPullRequest)
		const threadKey = reviewParent ? `${key}:${diffCommentLocationKey(reviewParent)}` : null
		submitOptimisticComment({
			key,
			optimistic: {
				_tag: "review-comment",
	...
	const submitCommentModal = () => {
		switch (commentModal.target.kind) {
			case "diff":
				...
				return
			case "issue":
				submitIssueCommentViaCommentModal()
				return
			case "reply":
				submitReplyComment()
				return
	...
	const executeMergeAction = (
		...
		if (targetPullRequest && kindDef.optimisticState === "merged") updatePullRequest(targetPullRequest.url, (pr) => ({ ...pr, state: "merged" as const }))
	...
	const commentAnchorLabel = ((): string => {
		if (commentModalActive) {
			if (commentModal.target.kind === "issue") return selectedIssue
				? `${selectedIssue.repository} #${selectedIssue.number}`
				: selectedPullRequest ? `New comment on #${selectedPullRequest.number}` : "New comment"
	...
	const activeCommentAnchorLabel = commentAnchorLabel
	...
	return (
		<box width={terminalWidth} height={terminalHeight} flexDirection="column" backgroundColor={colors.background}>
			...
			{isWideLayout && !detailFullView && !diffFullView && !commentsViewActive && !isInitialLoading ? (
			...
			{isInitialLoading ? (
						...
						{activeSurface === "issues" && isSelectedIssueDetailLoading && selectedIssue ? (
							...
							<>
								...
								<scrollbox key={`pull-body-${selectedPullRequest.url}`} ref={detailPreviewScrollRef} flexGrow={1} verticalScrollbarOptions={{ visible: wideDetailBodyScrollable }}>
									<DetailBody pullRequest={selectedPullRequest} contentWidth={rightContentWidth} bodyLines={wideDetailLines} bodyLineLimit={DETAIL_BODY_SCROLL_LIMIT} loadingIndicator={loadingIndicator} themeId={themeId} comments={selectedComments} commentsStatus={selectedCommentsStatus} onLinkOpen={openLinkInBrowser} />
								</scrollbox>
							</>
						) : (
							<DetailPlaceholder content={detailPlaceholderContent} paneWidth={rightPaneWidth} />
				...
				<box key="narrow-main" height={wideBodyHeight} flexDirection="column">
					{activeSurface === "issues" ? (
						...
						<DetailsPane pullRequest={selectedPullRequest} viewerUsername={username} contentWidth={fullscreenContentWidth} paneWidth={contentWidth} placeholderContent={detailPlaceholderContent} loadingIndicator={loadingIndicator} themeId={themeId} comments={selectedComments} commentsStatus={selectedCommentsStatus} onLinkOpen={openLinkInBrowser} />
					)}
					<Divider width={contentWidth} />
					<box flexGrow={1} flexDirection="column">
			...
			<box width={contentWidth} height={1} paddingLeft={1} paddingRight={1} backgroundColor={colors.background}>
				{footerNotice ? (
					...
					<FooterHints
						...
						isLoading={activeStatus === "loading" || (activeSurface === "issues" ? isRefreshingIssues || isHydratingIssueDetails : activeSurface === "pullRequests" ? isRefreshingPullRequests || isHydratingPullRequestDetails : isRefreshingAuxiliary) || closeModal.running || confirmActionModal.running || pullRequestStateModal.running || mergeModal.running || submitReviewModal.running}
						loadingIndicator={loadingIndicator}
